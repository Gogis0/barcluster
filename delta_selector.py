...
def delta_iterative_search(filename, delta_min, delta_max, delta_step, bucket_draft_size=5):
    ...
    with h5py.File(os.path.join(data_path, filename), 'r') as f:
        ...
        for barcode in barcode_list:
            for key in test_data[barcode]:
                signal = preprocess(np.array(f[barcode][key]).astype(float))
                matrix.append(signal) # TODO
        ldtw.ComputeMatrix(matrix, os.path.join(data_path, 'scoring_scheme.txt'), 10, 1, 1)
...
inter, cross = delta_iterative_search('barcoded_reads.hdf5', 0, 1000, 50, 50)
